...
// 空的？
// Groups中gid对应的[]string是什么
// 主要是加入新的group，然后需要将之前分配的shard比较多的均衡到相对比较少的
func (csm *CtrlerStateMachine) Join(groups map[int][]string) Err {
	num := len(csm.Configs)
	lastConfig := csm.Configs[num-1]
	// 构建新的配置
	// 首先定义为上一个Config的Shards和Groups
	newConfig := Config{
		Num:    num,
	...
	// 如果gid为0有shards那么需要先用这个gid，给这个gid分配shard
	// 需要首先对gidToShards中的gid排序，否则shardCtrl中的不同机器可能保存不同的配置
...
// Leave 相当于将一些group移除出去，同时需要重新分配这些group对应的shard到shard相对较少的group中
...
// Move 将一个shard移动到一个group中
// 为什么不需要维护config中Groups的变化？
